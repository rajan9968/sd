...
export default function EditCareer() {
    ...
    useEffect(() => {
        ...
        const fetchCareer = async () => {
            try {
                const response = await api.get(`/media-banner/get-media-bannerid/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/media-banner/updatemedia-banner/${id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            toast.success('Career updated successfully!');
            setTimeout(() => navigate('/admin/media-coverages-banner'), 1000);
        } catch (error) {
