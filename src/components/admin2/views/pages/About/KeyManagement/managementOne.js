...
export default function AboutBannerList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this banner?")) {
            api
                .delete(`/management/key-management/${id}`)
                .then((response) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row) => (
                <img
                    src={`${API_PATH}/uploads/key-management/${row.image}`}
                    alt={row.name}
                    width="80"
        ...
        },
        {
            name: "Instagram",
            cell: (row) => (
                <a href={row.insta} target="_blank" rel="noopener noreferrer">
                    {row.insta ? "View" : "-"}
                </a>
            ),
        },
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get("/management/all-key-management");
                    ...
                    <div className='row mb-3'>
                        <div className='col-lg-6 d-flex align-items-center'>
                            <CBreadcrumb className="mx-3 mb-0">
                                <CBreadcrumbItem>Management</CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                        <div className='col-lg-6 d-flex justify-content-end'>
                            <Link to="/admin/add-management" className='text-decoration-none'>
