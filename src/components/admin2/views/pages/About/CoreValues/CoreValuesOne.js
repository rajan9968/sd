...
export default function AboutBannerList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this banner?")) {
            api
                .delete(`/values/delete-add-core-values/${id}`)
                .then((response) => {
    ...
    const columns = [
        ...
        },
        {
            name: "Image",
            cell: (row) => (
                <img src={`${API_PATH}/uploads/our-culture/${row.banner_image}`} alt={row.sub_heading} style={{ width: '100px', height: 'auto' }} />
            ),
        },
        {
            name: "Content",
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get("/values/get-core-values");
                    ...
                    <div className='row mb-3'>
                        <div className='col-lg-6 d-flex align-items-center'>
                            <CBreadcrumb className="mx-3 mb-0">
                                <CBreadcrumbItem>Core Values</CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                        <div className='col-lg-6 d-flex justify-content-end'>
                            <Link to="/admin/add-core-values" className='text-decoration-none'>
