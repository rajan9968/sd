...
export default function AboutBannerList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this banner?")) {
            api
                .delete(`/project/delete-project-banner/${id}`)
                .then((response) => {
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="d-flex gap-2">
                    <Link
                        to="/admin/edit-project-portfolio"
                        state={{ id: row.id }}
                    >
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get("/project/get-project-banners");
                    ...
                    <div className='row mb-3'>
                        <div className='col-lg-6 d-flex align-items-center'>
                            <CBreadcrumb className="mx-3 mb-0">
                                <CBreadcrumbItem>Project Portfolio Banner</CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                        <div className='col-lg-6 d-flex justify-content-end'>
                            <Link to="/admin/add-project-portfolio" className='text-decoration-none'>
                                <CButton className='custom-button text-white align-items-center d-flex gap-2'>
                                    <CIcon icon={cilPlus} size="lg" />
                                    Add Project Portfolio Banner
                                </CButton>
                            </Link>
                        </div>
                    </div>
