...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await api.get(`/project/get-project-banner/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/project/update-project-banner/${id}`, formData, {
                headers: {
            ...
            if (response.data.success) {
                ...
                setTimeout(() => navigate('/admin/project-portfolio'), 1000);
            } else {
