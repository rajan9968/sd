...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await api.get(`/purpose/get-purpose/${id}`);
                if (response.data.success && response.data.banner) {
                    const banner = response.data.banner;
                    setSubHeading(banner.sub_heading);
                    setPurposeType(banner.purpose_type);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/purpose/update-purpose/${id}`, formData);

            if (response.data.success) {
                toast.success('About banner updated successfully!');
                setTimeout(() => navigate('/admin/purpose'), 1000);
            } else {
