...
export default function AboutBannerList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this banner?")) {
            api
                .delete(`/about-banner/delete-about-banner/${id}`)
                .then((response) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get("/leadership/get-leadership-banners");
                    ...
                    <div className='row mb-3'>
                        <div className='col-lg-6 d-flex align-items-center'>
                            <CBreadcrumb className="mx-3 mb-0">
                                <CBreadcrumbItem>Leadership Banner</CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                        <div className='col-lg-6 d-flex justify-content-end'>
                            <Link to="/admin/add-leadership" className='text-decoration-none'>
                                <CButton className='custom-button text-white align-items-center d-flex gap-2'>
                                    <CIcon icon={cilPlus} size="lg" />
                                    Add Leadership Banner
                                </CButton>
                            </Link>
                        </div>
                    </div>
