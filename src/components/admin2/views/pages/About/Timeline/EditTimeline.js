...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get(`/timeline/get-timelines/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/timeline/update-timelines/${id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            if (response.data.success) {
                toast.success('About banner updated successfully!');
                setTimeout(() => navigate('/admin/timelines'), 1000);
            } else {
