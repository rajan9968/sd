...
export default function EditCareer() {
    ...
    useEffect(() => {
        ...
        const fetchCareer = async () => {
            try {
                const response = await api.get(`/faqs/get-faqs-bannerid/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/faqs/updatefaqs-banner/${id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            toast.success('Career updated successfully!');
            setTimeout(() => navigate('/admin/faqs-banner'), 1000);
        } catch (error) {
