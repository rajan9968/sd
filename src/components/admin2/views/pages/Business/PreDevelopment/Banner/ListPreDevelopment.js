...
export default function AboutBannerList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this banner?")) {
            api
                .delete(`/pre-development/delete-pre-development-banner/${id}`)
                .then((response) => {
    ...
    const columns = [
        ...
        },
        {
            name: "Overview Text",
            selector: (row) => {
                if (!row.overview_text) return "-";
                const words = row.overview_text.split(" ");
...

        {
            ...
            cell: (row) =>
                row.overview_image ? (
                    <img
                        src={`${API_PATH}/uploads/about-banners/${row.overview_image}`}
                        alt="Overview"
                        width="100"
                        style={{ borderRadius: "8px" }}
                    />
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="d-flex gap-2">
                    <Link
                        to="/admin/edit-pre-development-epc"
                        state={{ id: row.id }}
                    >
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await api.get("/pre-development/get-pre-development-banners");
                    ...
                    <div className='row mb-3'>
                        <div className='col-lg-6 d-flex align-items-center'>
                            <CBreadcrumb className="mx-3 mb-0">
                                <CBreadcrumbItem>Business</CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                        <div className='col-lg-6 d-flex justify-content-end'>
                            <Link to="/admin/add-pre-development-epc" className='text-decoration-none'>
