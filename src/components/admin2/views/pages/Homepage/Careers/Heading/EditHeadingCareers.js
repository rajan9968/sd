...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await api.get(`/careers-heading/get-careers-heading/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/careers-heading/update-careers-heading/${id}`, formData);

            if (response.data.success) {
                toast.success('Careers heading updated successfully!');
                setTimeout(() => navigate('/admin/careers-heading'), 1000);
            } else {
