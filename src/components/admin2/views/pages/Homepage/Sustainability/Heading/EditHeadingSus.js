...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await api.get(`/sustainability-heading/get-sustainability-heading/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/sustainability-heading/update-sustainability-heading/${id}`, formData);

            if (response.data.success) {
                toast.success('Sustainability heading updated successfully!');
                setTimeout(() => navigate('/admin/list-heading-sus'), 1000);
            } else {
