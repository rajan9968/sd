...
export default function AddPortfolio() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.post('/culture-portfolio/add-portfolio', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            if (response.data.success) {
                toast.success('Portfolio record created successfully!');
                setTimeout(() => {
                    navigate('/admin/culture-portfolio-list'); // adjust to list page
                }, 1000);
            } else {
                    ...
                    <div className='row bg-white mx-1 mb-3 py-3 shadow-sm'>
                        <div className='col-lg-12 d-flex align-items-center'>
                            <CBreadcrumb className="mb-0">
                                <CBreadcrumbItem>
                                    <Link to="/admin/dashboard" className='text-decoration-none'>Home</Link>/Add Culture Portfolio
                                </CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                    </div>
