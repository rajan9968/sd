...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await api.get(`/portfolio-heading/get-portfolio-heading/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/portfolio-heading/update-portfolio-heading/${id}`, formData);

            if (response.data.success) {
                toast.success('Portfolio heading updated successfully!');
                setTimeout(() => navigate('/admin/portfolio-heading'), 1000);
            } else {
