...
export default function EditCareer() {
    ...
    useEffect(() => {
        ...
        const fetchCareer = async () => {
            try {
                const response = await api.get(`/press-banner/get-press-bannerid/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/press-banner/updatepress-banner/${id}`, formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            toast.success('Career updated successfully!');
            setTimeout(() => navigate('/admin/press-release-banner'), 1000);
        } catch (error) {
                        ...
                        <CForm className="row g-3" onSubmit={handleSubmit}>
                            ...
                            {existingImage && (
                                <CCol md={6} className="mb-2">
                                    ...
                                    <div>
                                        <img
                                            src={`http://localhost:50000/uploads/careers/${existingImage}`}
                                            alt="career"
                                            width="100"
                                        />
                                    </div>
