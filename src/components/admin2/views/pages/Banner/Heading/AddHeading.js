...
export default function AddAboutBanner() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.post('/business-heading/add-business-heading', formData);

            if (response.data.success) {
                toast.success('Business Heading created successfully!');

                setTimeout(() => {
                    navigate('/admin/list-heading'); // Redirect to business heading list
                }, 1000);
            } else {
                    ...
                    <div className='row bg-white mx-1 mb-3 py-3 shadow-sm'>
                        <div className='col-lg-12 d-flex align-items-center'>
                            <CBreadcrumb className="mb-0">
                                <CBreadcrumbItem>
                                    <Link to="/admin/dashboard" className='text-decoration-none'>Home</Link>/Add Business Heading
                                </CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                    </div>
