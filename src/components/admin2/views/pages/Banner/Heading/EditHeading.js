...
export default function EditAboutBanner() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                const response = await api.get(`/business-heading/get-business-heading/${id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.put(`/business-heading/update-business-heading/${id}`, formData);

            if (response.data.success) {
                toast.success('About banner updated successfully!');
                setTimeout(() => navigate('/admin/list-heading'), 1000);
            } else {
