...
export default function AddAboutBanner() {
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('cat_id', selectedCategory); // ✅ FIXED
        formData.append('status', status);
        console.log(formData);
        try {
            const response = await api.post('/sub-cat/add-category', formData);

            if (response.data.success) {
                toast.success('category created successfully!');

                setTimeout(() => {
                    navigate('/admin/sub-category-list'); // Redirect to portfolio heading list
                }, 1000);
            } else {
                    ...
                    <div className='row bg-white mx-1 mb-3 py-3 shadow-sm'>
                        <div className='col-lg-12 d-flex align-items-center'>
                            <CBreadcrumb className="mb-0">
                                <CBreadcrumbItem>
                                    <Link to="/admin/dashboard" className='text-decoration-none'>Home</Link>/Add Sub Category
                                </CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                    </div>
