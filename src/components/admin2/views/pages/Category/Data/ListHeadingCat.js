...
export default function AboutBannerList() {
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete this banner?")) {
            api
                .delete(`/investor/delete-investor/${id}`)
                .then((response) => {
    ...
    const columns = [
        ...
        {
            name: "Title",
            selector: row => row.title, // ✅ updated: use "title" from API
            sortable: true,
        },
        {
            name: "Year",
            selector: row => row.year, // ✅ added "year" column
            sortable: true,
        },
        {
            name: "Category",
            selector: (row) => {
                const selectedCategory = categories.find(
                    (category) => category.id === Number(row.cat_id)
                );
                return selectedCategory ? selectedCategory.sub_heading : "Unknown";
            },
            sortable: true,
        },
        {
            name: "Link",
            selector: row => row.link, // ✅ added "year" column
            sortable: true,
        },
        {
            name: "Action",
        ...
        const fetchData = async () => {
            try {
                ...
                if (response.data.success && response.data.subCategories) {
                    setData(response.data.subCategories); // ✅ updated key
                } else {
                    ...
                    <div className='row mb-3'>
                        <div className='col-lg-6 d-flex align-items-center'>
                            <CBreadcrumb className="mx-3 mb-0">
                                <CBreadcrumbItem>Investors</CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                        <div className='col-lg-6 d-flex justify-content-end'>
                            <Link to="/admin/add-investors" className='text-decoration-none'>
