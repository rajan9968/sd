...
export default function AddAboutBanner() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.post('/cat/add-category', formData);

            if (response.data.success) {
                toast.success('category created successfully!');

                setTimeout(() => {
                    navigate('/admin/category-list'); // Redirect to portfolio heading list
                }, 1000);
            } else {
                    ...
                    <div className='row bg-white mx-1 mb-3 py-3 shadow-sm'>
                        <div className='col-lg-12 d-flex align-items-center'>
                            <CBreadcrumb className="mb-0">
                                <CBreadcrumbItem>
                                    <Link to="/admin/dashboard" className='text-decoration-none'>Home</Link>/Add Category
                                </CBreadcrumbItem>
                            </CBreadcrumb>
                        </div>
                    </div>
