...
export default function AddCareer() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await api.post('/case-studies-banner/add-case-studies-banner', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });

            toast.success('Case Studies Banner added successfully!');
            setTimeout(() => navigate('/admin/case-studies-banner'), 1000);
        } catch (error) {
            console.error('Error adding blog banner:', error);
            toast.error(error.response?.data?.message || 'Failed to add blog banner');
        }
    };
